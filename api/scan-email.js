...
function detectarTipo(subject = '', snippet = '') {
...
export default async function handler(req, res) {
  // Segurança: só aceita chamada do cron Vercel ou GET manual (para teste)
  ...
  try {
    // 1. Buscar refresh_token salvo no Supabase
    const config = await sbGet('configuracoes', '?id=eq.gmail_refresh_token&select=dados');
    const refreshToken = config[0]?.dados?.token;
    if (!refreshToken) {
      return res.status(400).json({ error: 'Gmail não autorizado. Acesse /api/auth/gmail primeiro.' });
    }
    ...
    // 3. Buscar timestamp da última varredura
    ...
    const afterUnix = lastScanTs
      ...
      : Math.floor((Date.now() - 25 * 60 * 60 * 1000) / 1000); // últimas 25h se primeira vez
    ...
    // 5. Buscar IDs já processados para evitar duplicatas
    ...
    // 6. Buscar serviços para cruzar número do processo
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        // Extrair número(s) de processo CNJ
        const texto = subject + ' ' + snippet;
        const matches = texto.match(PROC_REGEX);
        const numProc = matches?.[0] || null;
        ...
        const tipo = detectarTipo(subject, snippet);
    ...
    // 8. Atualizar timestamp da última varredura
    await atualizarUltimaVarredura();
